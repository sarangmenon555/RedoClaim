...
export default function AppealsPage() {
  ...
  return (
    <div className="max-w-5xl space-y-6 animate-fade-in" style={{color:"var(--text-primary)"}}>
      <div>
        <h2 className="text-2xl font-bold style-text-primary">{t("ap_title")}</h2>
        <p className="style-text-tertiary text-sm mt-1">
          {t("ap_subtitle")} <strong>{t("ap_review_carefully")}</strong>
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          <div className="card p-4">
            <label className="label">{t("ap_select_claim")}</label>
            {claims.length === 0 ? (
              ...
              <select className="input" value={selectedClaimId}
                ...
                <option value="">{t("ap_choose_claim")}</option>
                {claims.map((c) => (
            ...
            {selectedClaim && (
              <div className="mt-3 p-3 bg-surface-2 rounded-lg text-xs space-y-1">
                <p className="font-semibold style-text-secondary mb-1">{t("ap_claim_details")}</p>
                <div className="flex justify-between">
                  <span className="style-text-tertiary">{t("ap_insurer")}</span>
                  <span className="style-text-primary font-medium">{selectedClaim.insurer_name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="style-text-tertiary">{t("ap_policy_no")}</span>
                  <span className={`font-medium ${selectedClaim.policy_number ? "style-text-primary" : "text-amber-600"}`}>
                    {selectedClaim.policy_number || t("not_set")}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="style-text-tertiary">{t("ap_claim_amount")}</span>
                  <span className={`font-medium ${selectedClaim.claim_amount ? "style-text-primary" : "text-amber-600"}`}>
                    {selectedClaim.claim_amount ? `Rs. ${selectedClaim.claim_amount.toLocaleString("en-IN")}` : t("not_set")}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="style-text-tertiary">{t("ap_rejection_date")}</span>
          ...
          {/* Appeal type */}
          <div className="card p-4">
            <label className="label mb-3 block">{t("ap_letter_type")}</label>
          ...
          {/* Context */}
          <div className="card p-4">
            <label className="label">{t("ap_additional_details")}</label>
          ...
          <button onClick={generateAppeal} disabled={generating || !selectedClaimId}
            ...
            {generating
              ? <><Loader2 size={15} className="animate-spin" /> {t("ap_drafting")}</>
              : <><FileText size={15} /> {t("ap_generate_draft")} <ArrowRight size={13} /></>}
          </button>
        ...
        <div className="md:col-span-2">
          {!letter && !generating && (
            <div className="card p-12 text-center h-full flex flex-col items-center justify-center gap-4">
              ...
              <div>
                <p className="font-semibold style-text-secondary">{t("ap_letter_placeholder")}</p>
                <p className="text-sm style-text-tertiary mt-1">{t("ap_ai_generated_note")}</p>
              </div>
            </div>
          )}
          ...
          {generating && (
            <div className="card p-12 text-center h-full flex flex-col items-center justify-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm style-text-tertiary mt-1">{t("ap_composing")}</p>
              </div>
            </div>
          )}
            ...
            <div className="card overflow-hidden">
              ...
              <div className="p-4 border-b border-surface-4 flex items-center justify-between bg-surface-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium style-text-primary text-sm">
                    {selectedAppeal?.label} — {t("ap_ai_draft")}
                  </span>
                  <AIOutputLabel />
                </div>
              ...
              <div className="p-4 bg-surface-2 border-b border-amber-500\/20">
                <p className="text-xs font-semibold text-amber-800 mb-2 flex items-center gap-1.5">
                  <AlertTriangle size={12} /> {t("ap_review_checklist")}
                </p>

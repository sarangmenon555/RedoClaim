...
export default function CISPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-in" style={{color:"var(--text-primary)"}}>
      <div>
        <h2 className="text-2xl font-bold style-text-primary">{t("cis_title")}</h2>
      ...
      <div className="card p-4 bg-surface-2 border-amber-500\/20 flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold text-amber-800 text-sm">{t("cis_what_title")}</p>
      ...
      {!docId && (
        <div {...getRootProps()} className={`card border-2 border-dashed p-10 text-center cursor-pointer transition-all ${isDragActive ? "border-violet-400 bg-surface-2" : "border-surface-5 hover:border-violet-400"}`}>
          <input {...getInputProps()} />
          {uploading ? (
            <div className="flex flex-col items-center gap-3"><Loader2 className="animate-spin text-violet-400" size={32} /><p className="font-medium style-text-secondary">{t("cis_processing")}</p></div>
          ) : (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="font-semibold style-text-primary">{t("cis_upload_title")}</p>
              <p className="text-sm style-text-tertiary">{t("cis_pdf_or_image")}</p>
            </div>
          )}
        </div>
      )}
      ...
      {docId && !result && (
        <div className="card p-5 text-center space-y-4">
          ...
          <p className="font-medium style-text-primary">{t("cis_ready")}</p>
          <button onClick={scanCIS} disabled={scanning} className="btn-primary">
            {scanning ? <><Loader2 size={15} className="animate-spin" /> {t("cis_scanning")}</> : <>{t("cis_scan_btn")}</>}
          </button>
        </div>
      )}
      ...
      {cis && (
        <div className="space-y-4 animate-slide-up">
          <div className="card p-5 border-l-4 border-green-500">
            ...
            <p className="text-sm style-text-tertiary">{t("cis_sum_insured")} {cis.sum_insured}</p>
          </div>

          {/* Plain English Summary */}
          <div className="grid md:grid-cols-2 gap-4">
            <div className="card p-5 border-l-4 border-green-400">
              <div className="flex items-center gap-2 mb-2"><CheckCircle size={15} className="text-green-600" /><p className="font-semibold style-text-primary">{t("cis_covered")}</p></div>
            ...
            <div className="card p-5 border-l-4 border-red-400">
              <div className="flex items-center gap-2 mb-2"><AlertTriangle size={15} className="text-red-600" /><p className="font-semibold style-text-primary">{t("cis_not_covered")}</p></div>
          ...
          {cis.risky_exclusions?.length > 0 && (
            <div className="card overflow-hidden">
              <div className="p-4 bg-surface-2 border-b border-red-500\/20 flex items-center gap-2">
                ...
                <span className="font-semibold text-red-800 text-sm">{t("cis_risky_exclusions")}</span>
              </div>
          ...
          {cis.inclusions?.length > 0 && (
            <div className="card p-5">
              <p className="font-semibold style-text-primary mb-3">{t("cis_all_inclusions")} ({cis.inclusions.length})</p>
